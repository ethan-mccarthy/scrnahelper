...
makeFeaturePlots <- function(seuratObject, csvFile){
  ...
  featurePlotDir <- paste("FeaturePlots_", substitute(seuratObject), sep = "")
  ...
  for (i in 1:nrow(df)) {
    marker <- df[i, "Markers"]
    plot <- Seurat::FeaturePlot(seuratObject, features = marker)
    plot_list[[i]] <- plot
  }
